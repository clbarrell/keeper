...
chrome.tabs.onActivated.addListener((t) => {
  ...
  chrome.tabs.get(currentTabId, (res) => {
    if (/moodModal|chrome:\/\/|chrome-extension:\/\//.test(res.url)) {
      ...
      chrome.storage.local.get(["tabChanges", "changeThreshold", "lastAlert"], function (result) {
...
const debug = () => {
  chrome.storage.local.get(["tabChanges", "tabActivity", "moodRatings", "lastAlert"], function (result) {
